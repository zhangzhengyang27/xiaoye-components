...
> 本篇是「令牌体系」章节的收尾篇。核心问题只有一个：**一处组件实测出来的字重值，如何走完"写死 → 沉淀 → 转正 → 全库替换"四步，从某个 CSS 文件里的一行字面量，变成全库组件共享的设计资产？** 我们拿按钮上那个 `560` 当解剖标本，把 `tokens.css` 的字重段、TS 常量层的镜像、生成脚本的归类逻辑和 47 个组件样式文件的替换现场逐一翻出来看——所有数字都在当前工作区重新核对过，行号可查。
...
先把"战前"的状态钉死。所谓战前，指当前工作区改动之前的最近一次提交 `7645d5c`——它把令牌体系重构为 Stripe 设计语言，字重段在那时建立，但只有 4 个命名档（`git show HEAD:packages/xiaoye-primitives/src/theme/tokens.css` 的 L211-214：light 300 / regular 400 / medium 500 / semibold 600，全文件 401 行）。如果再往前翻到 `3124113`（`tokens.css` 在 Stripe 重构前的上一次变更），文件只有 164 行，**一个字重档都没有**——字重这个维度在那个时代完全不存在于令牌层。
...
把结果按值归并，战前全库字重的真实分布是这样一张表（每个数字都在当前工作区用 `git grep` + `git diff` 复核过）：
...
12 档。对照 HEAD 版本的同一位置，能看清这次扩档的完整增量——这是当前**未提交工作区**里的 diff（`git diff -- packages/xiaoye-primitives/src/theme/tokens.css`）：
...
替换还顺带完成了一次"长尾清剿"。战前那批只有一个据点的孤值，如今全部有了出处（以下均为当前工作区行号）：
