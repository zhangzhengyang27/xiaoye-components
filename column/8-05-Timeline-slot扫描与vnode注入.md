...
下一篇预告：8-06《Scheduler：fullcalendar 集成》。timeline 选择了"父渲染子"来处理一组同构子项，下一篇换一个更重的样本——fullcalendar 这个第三方日历引擎如何被装进 `XyScheduler` 的组件外壳：委托式生命周期桥、props 到 options 的映射层、事件回调到 Vue emit 的桥接、以及约 250 行自研 RRULE 展开为什么没有被交给 rrule 依赖。第三方库的组件化封装边界，"数据快照、能力通行"四维框架将在那里正式立起来。
