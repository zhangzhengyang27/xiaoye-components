...
增强层现在有多少货？答案写在 `packages/pro-components/component-manifest.json` 里——31 个条目，覆盖五大业务分组：form（增强表单 9 个）、page（增强页面 10 个）、data（增强数据 5 个）、detail（增强详情 3 个）、workflow（增强流程 3 个），外加穿针引线的 misc 位置若干。每个条目五个字段，先用 form 组开头的原文看结构：
...
```ts
// packages/pro-components/index.ts（L1-46：头部与类型白名单开篇）
...
先看类型守卫，因为它直接兑现了第四节的破坏性代价。`tests/types/fixtures/pro-root-boundary.ts` 全文 44 行：
...
再看脚本守卫。`scripts/check-pro-components.mjs` 全文 332 行，本库的脚本守卫一贯是"零依赖 Node 直读文件 + 正则抠事实 + 双向 diff"的风格。第一段事实是那份手写的根入口类型白名单预期表：
