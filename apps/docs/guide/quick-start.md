...
这页先帮你用最短路径把 `xiaoye-components` 接进 Vue 3 项目，并尽快跑通第一个页面。

<p class="xy-section-lead">
  如果你只是想先把组件库接进现有后台项目，不需要一开始就做复杂拆分。先完整引入、跑通页面，再决定要不要细化到按组件引入，会更稳。
</p>
...
- 如果你在做新项目，先完整引入，把页面节奏跑通。
- 如果你在接已有后台项目，再按组件引入，让依赖关系更清楚。
