...
`xy-skeleton` 用于承接接口尚未返回、页面结构已知但内容仍在加载中的状态。它参考了 `element-plus` 的 `Skeleton` 方案，适合列表页、卡片区和详情块这类“先有结构，再等数据”的中后台场景。
...
:::demo `animated` 适合强调“正在加载中”，`rows` 和 `count` 则更适合批量列表或多块卡片一起占位的场景。
...
- 对同一区域的连续加载，优先保持骨架结构稳定，不要频繁切换不同占位形态。
