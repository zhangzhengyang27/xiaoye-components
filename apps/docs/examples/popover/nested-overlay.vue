    ...
    <xy-popover v-model="popoverOpen" title="发布前确认" width="360px">
      ...
      <div class="xy-doc-stack">
        ...
        <xy-space>
          ...
          <xy-button type="warning" @click="escalateToModal">升级到弹窗确认</xy-button>
        </xy-space>
      </div>
    </xy-popover>

    <xy-modal v-model="modalOpen" title="发布风险确认">
      <p>这里适合承载更完整的阻断式确认，例如责任人、影响范围和回滚方案。</p>

      <template #footer>
        <xy-space>
          <xy-button plain @click="modalOpen = false">取消</xy-button>
          <xy-button type="primary" @click="modalOpen = false">确认发布</xy-button>
        </xy-space>
      </template>
    </xy-modal>
  </div>
