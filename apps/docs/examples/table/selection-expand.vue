...
<template>
  <div class="xy-doc-stack">
    <xy-space wrap>
      <xy-button size="sm" @click="tableRef?.toggleAllSelection()">全选当前页</xy-button>
      <xy-button size="sm" @click="tableRef?.clearSelection()">清空选择</xy-button>
      <xy-button size="sm" @click="tableRef?.clearSort()">清空排序</xy-button>
    </xy-space>
    ...
    >
      ...
      <xy-table-column type="expand" width="60">
        <template #default="{ row }">
          <div class="table-demo-expanded">
            <xy-tag status="primary">{{ row.name }}</xy-tag>
      ...
      <xy-table-column prop="score" label="健康度" align="right" sortable min-width="120" fixed="right" />
    </xy-table>
  </div>
</template>
